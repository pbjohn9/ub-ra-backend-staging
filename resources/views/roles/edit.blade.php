                        ...
                        <form action="{{ route('roles.update',$role->id) }}" method="post">
                            ...
                            <div class="row">
                                <div class="col-12 form-group">
                                    ...
                                    <input type="text" class="form-control @error('title') is-invalid @enderror" value="{{ old('title',$role->title) }}" name="title" placeholder="Enter title">
                                    @error('title')
                                ...
                                <div class="col-12 form-group">
                                    ...
                                    <select class="select2 form-control select2-multiple @error('permissions') is-invalid @enderror" multiple="multiple" name="permissions[]" multiple data-placeholder="Choose ...">
                                        @foreach ($permissions as $id=>$title)
                                            <option value="{{ $id }}" {{ (in_array($id, old('permissions', [])) || $role->permissions->contains($id)) ? 'selected' : '' }}>{{ $title }}</option>
                                        @endforeach
                                    </select>
                                    @error('permissions')
                                        <span class="invalid-feedback" role="alert">
